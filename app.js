...
function renderStickies() {
  ...
  state.stickies.forEach((note, idx) => {
    ...
    div.innerHTML = `
      <div class="sticky-author">${note.author} →</div>
...
function renderArchive() {
  ...
  todayItem.innerHTML = `
    <span class="archive-date">${state.date}</span>
    <span class="archive-mood">${state.mood_meow} ♡ ${state.mood_vein}</span>
  `;
  ...
  state.archive.forEach(item => {
    ...
    div.innerHTML = `
      ...
      <span class="archive-mood">${item.mood_meow} ♡ ${item.mood_vein}</span>
    `;
