...
async function loadDay(date) {
  ...
  if (token) {
    try {
      const res = await githubGet(`${DIARY_FOLDER}/${date}.json`);
      if (res && !res.notFound) {
        data = JSON.parse(base64ToUtf8(res.content));
        sha = res.sha;
      }
    } catch (e) {
      console.error('云端加载失败，用本地', e);
    }
  }
  ...
  currentDay = data;
  currentSha = sha;
  setSyncStatus(token ? '已同步 ♡' : '本地保存中', token ? 'synced' : '');
  renderAll();
}
...
function renderArchive() {
  ...
  allDates.forEach(date => {
    ...
    const moodMeow = data?.mood_meow || '·';
    const moodVein = data?.mood_vein || '·';
    div.innerHTML = `<span class="archive-date">${date}</span> <span class="archive-mood">${moodMeow} ♡ ${moodVein}</span>`;
